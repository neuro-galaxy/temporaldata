...
### Deprecated
- Deprecated `set_train_domain`, `set_valid_domain`, and `set_test_domain` methods in `Data`. 
- Deprecated `_check_for_data_leakage` method in `Data`. 

### Removed
- Removed `add_split_mask` method from `Data`, `Interval`, `IrregularTimeSeries`, and `RegularTimeSeries` classes.
- Removed `allow_split_mask_overlap` method from `Interval`.
